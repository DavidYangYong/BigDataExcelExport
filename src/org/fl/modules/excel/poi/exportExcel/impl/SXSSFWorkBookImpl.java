...
public class SXSSFWorkBookImpl implements ISXSSFWorkBook {
		...
		for (int i = 0; i < excelParams.size(); i++) {
			...
			try {
				contentCell.setCellValue(
						getValueStr(getCellValue(excelExportEntity, object)));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private static Object getCellValue(ExcelExportEntity entity, Object obj)
			throws Exception {
		Object value = entity.getGetMethods() != null
				? getFieldBySomeMethod(entity.getGetMethods(), obj)
	...
	private static void getAllExcelField(String targetId, Field[] fields, List<ExcelExportEntity> excelParams, Class<?> pojoClass, List<Method> getMethods)
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (ExcelPublicUtil.isCollection(field.getType())) {
				...
				getExcelFieldList(targetId, ExcelPublicUtil.getClassFields(clz),
						clz, list, null);
				excelEntity = new ExcelExportEntity();
				excelEntity.setName(getExcelName(excel.exportName(), targetId));
				excelEntity
						.setOrderNum(getCellOrder(excel.orderNum(), targetId));
				excelEntity.setGetMethod(
						ExcelPublicUtil.getMethod(field.getName(), pojoClass));
	...
	public static void getExcelField(String targetId, Field field, ExcelExportEntity excelEntity, Excel excel, Class<?> pojoClass)
		...
		excelEntity
				.setGetMethod(ExcelPublicUtil.getMethod(fieldname, pojoClass));
	...
	public void doExecuteCreateTitle(Sheet sheet) {
		...
		Cell contentCell = contenRow.createCell(0);
		int cellIndex = 0;
		for (int i = 0, exportFieldTitleSize = excelParams
				.size(); i < exportFieldTitleSize; i++) {
			ExcelExportEntity entity = excelParams.get(i);
			contentCell.setCellValue(entity.getName());
			sheet.autoSizeColumn(i);
			cellIndex++;
			contentCell = contenRow.createCell(cellIndex);
